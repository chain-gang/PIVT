...
const contractName = 'comm-paper';
const orgHost = 'atlantis.com';
const channelName = 'private-karga-atlantis';

const CommercialPaper = require(`../${contractName}/lib/paper.js`);
...
async function main() {
  ...
  try {
    ...
    const userName = `Admin@${orgHost}`;

    // Load connection profile; will be used to locate a gateway
    let connectionProfile = yaml.safeLoad(fs.readFileSync('../gateway.yaml', 'utf8'));
    ...
    const contract = await network.getContract(contractName, 'org.papernet.commercialpaper');
